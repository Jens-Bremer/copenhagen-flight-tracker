...
PRICES_VOLATILE = [100, 500, 900, 100, 500]
PRICES_STABLE = [200, 210, 205, 215, 208]
PRICES_ROLLING = [10, 12, 11, 13, 10, 12, 11, 13, 12, 10]
...
def test_cv_bars_sorted_descending_by_cv():
    rows = [
        _row(p, airline="Norwegian", obs_day=i % 3)
        for i, p in enumerate(PRICES_VOLATILE)
    ] + [_row(p, airline="KLM", obs_day=i % 3) for i, p in enumerate(PRICES_STABLE)]
    out = _build(rows, min_obs_for_cv=3, min_window_samples=2)
...
def test_multi_airline_string_excluded():
    rows = [
        _row(p, airline="KLM,Air France", obs_day=i)
...
def test_rolling_stddev_fields():
    rows = [_row(p * 100, obs_day=i) for i, p in enumerate(PRICES_ROLLING)]
    out = _build(rows, min_obs_for_cv=3, min_window_samples=2)
    pts = out["by_route"]["CPH-AMS"]["rolling_stddev"]
    assert len(pts) > 0
    for pt in pts:
        required = {"airline", "obs_date", "stddev_cents", "daily_min_cents", "n"}
        assert required <= pt.keys()
        assert pt["n"] >= 2
