...
function renderAirlineLeadTime() {
  destroyChart('airlineLeadtime');

  const routes = activeRoutes().filter((r) => DATA.analysis[r]);
  if (routes.length === 0) {
    const legendEl = $('airline-leadtime-legend');
  ...
  // Collect airlines with sufficient observations across all active routes.
  const airlineTotals = {};
  routes.forEach((r) => {
    (DATA.analysis[r].lead_time_curve || []).forEach((entry) => {
      Object.entries(entry.by_airline || {}).forEach(([airline, stats]) => {
        if (stats.obs_count >= MIN_OBS) {
          airlineTotals[airline] = (airlineTotals[airline] || 0) + stats.obs_count;
        }
      });
    });
  });

  // Top 8 airlines by total observation count.
  const topAirlines = Object.keys(airlineTotals)
    .sort((a, b) => airlineTotals[b] - airlineTotals[a])
    .slice(0, 8);
  ...
  // Initialise visibility state on first render.
  if (!state.airlineLeadTimeVisible) {
    state.airlineLeadTimeVisible = new Set(topAirlines);
  }

  // Render airline toggle pills.
  const legendEl = $('airline-leadtime-legend');
  if (legendEl) {
    legendEl.innerHTML = '';
    topAirlines.forEach((airline) => {
      ...
      pill.type = 'button';
      const isActive = state.airlineLeadTimeVisible.has(airline);
      pill.className = 'filter-chip';
      pill.style.borderColor = airlineColor(airline);
      if (isActive) {
        pill.style.backgroundColor = hexToRgba(airlineColor(airline), 0.15);
      ...
      pill.textContent = airline;
      pill.addEventListener('click', () => {
        if (state.airlineLeadTimeVisible.has(airline)) {
          state.airlineLeadTimeVisible.delete(airline);
        } else {
          state.airlineLeadTimeVisible.add(airline);
        }
  ...
  const visibleAirlines = topAirlines.filter((a) => state.airlineLeadTimeVisible.has(a));

  // Build datasets: per (route, airline) — Q1 anchor, IQR fill, median line.
  const datasets = routes.flatMap((r) => {
    const curve = DATA.analysis[r].lead_time_curve || [];
    const routeIdx = DATA.metadata.routes.indexOf(r);
    const routeSuffix = DATA.metadata.routes.length > 1 ? ` (${r})` : '';

    return visibleAirlines.flatMap((airline) => {
      const color = airlineColor(airline);
      const pts = curve.filter((e) => (e.by_airline?.[airline]?.obs_count ?? 0) >= MIN_OBS);
      if (pts.length === 0) return [];

      return [
        {
          label: `${airline}${routeSuffix} Q1`,
          data: pts.map((e) => ({ x: e.days_before, y: e.by_airline[airline].q1_cents / 100 })),
          borderColor: hexToRgba(color, 0),
          backgroundColor: hexToRgba(color, 0),
          fill: false, pointRadius: 0, spanGaps: false,
        },
        {
          label: `${airline}${routeSuffix} IQR`,
          data: pts.map((e) => ({ x: e.days_before, y: e.by_airline[airline].q3_cents / 100 })),
          borderColor: hexToRgba(color, 0),
          backgroundColor: hexToRgba(color, 0.12),
          fill: '-1', pointRadius: 0, spanGaps: false,
        },
        {
          label: `${airline}${routeSuffix}`,
          data: pts.map((e) => ({ x: e.days_before, y: e.by_airline[airline].median_cents / 100 })),
          borderColor: color,
          borderDash: routeIdx % 2 === 1 ? [6, 4] : [],
  ...
  charts.airlineLeadtime = new Chart($('airline-leadtime-chart'), {
    type: 'line',
    data: { datasets },
    options: {
      responsive: true, maintainAspectRatio: false,
      plugins: {
        title: {
          display: true,
          text: 'Per-airline median price by days before departure (IQR band, ≥5 obs per bucket)',
        },
        legend: {
          labels: {
            filter: (item) =>
              !item.text.endsWith(' Q1') && !item.text.endsWith(' IQR'),
          },
        },
        tooltip: {
          callbacks: {
            label: (ctx) => {
              if (ctx.dataset.label.endsWith(' Q1') || ctx.dataset.label.endsWith(' IQR')) return null;
              return `${ctx.dataset.label}: €${ctx.parsed.y.toFixed(0)} median`;
            },
          },
        },
      },
